...
def total_quotes_by(authorName):
    getCountOfQuotesByAuthor_Command = """SELECT COUNT(*) FROM quotes NATURAL JOIN authors WHERE name= ?""" 
...
def analyse_tags_count():
    getAnalysisOfTags_Command = """SELECT MIN(tagCount), MAX(tagCount), ROUND(AVG(tagCount),2) FROM (SELECT COUNT(tag_id) AS tagCount FROM quote_tag GROUP BY quote_id)""" 
...
def get_top_n_no_of_authors(N):
    getAnalysisOfTags_Command = """SELECT name AS author FROM quotes NATURAL JOIN authors GROUP BY author_id ORDER BY COUNT(name) DESC LIMIT ? """ 
...
def get_tags_of_author(author):
    getAnalysisOfTags_Command = """SELECT tag FROM (tags NATURAL JOIN quote_tag) NATURAL JOIN (quotes NATURAL JOIN authors) WHERE name=?""" 
